...
# 取得対象のWith×STATION URL
URLS = [
    "https://wikiwiki.jp/llll_wiki/With%C3%97STATION"
]
...
def main():
    ...
    for url in URLS:
        # URLからデコードしてページ名を取得（With×STATION）
        page_name = urllib.parse.unquote(url.split('/')[-1])
        print(f"\n[{page_name}] のデータを取得中...")
                ...
                for ul in ul_elements:
                    ...
                    for li in li_elements:
                        ...
                        if title_full and (youtube_url or participants or date_str):
                            video_data = {
                                "id": str(uuid.uuid4()),
                                "season": page_name, # With×MEETSのように期ごとのページ分割がない想定
                                "type": "with_station", # 判別用のtypeを変更
                                "date": date_str,
                                "title": title_str,
    ...
    # 別のフォルダ（dataディレクトリ）にJSONを保存する設定
    output_dir = 'data'
    os.makedirs(output_dir, exist_ok=True)
    output_filename = os.path.join(output_dir, 'withstation_wiki_data.json')
