...
export const FriendRankComparison = () => {
  ...
  return (
    <>
        ...
        <div className="space-y-3 flex-grow overflow-y-auto custom-scrollbar pr-2">
          {sortedUsers.map((user, index) => {
            ...
            return (
              <div 
                key={user.id} 
                // 🌟 クリックでダッシュボードを開く（'me'の場合は自分のIDを渡さないことで自分のデータを表示）
                onClick={() => setSelectedUser({uid: user.isMe ? '' : user.id, name: user.name})}
                className={`flex items-center justify-between p-3 rounded-lg border cursor-pointer hover:shadow-sm transition-all ${
                  user.isMe ? 'border-blue-200 bg-blue-50/50 hover:bg-blue-100/50' : 'border-gray-100 bg-gray-50 hover:bg-gray-100'
                }`}
              >
      ...
      {selectedUser && (
        <PointDashboard 
          targetUserId={selectedUser.uid || undefined}
