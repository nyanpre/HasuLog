...
export const VideoList = ({ title = "動画一覧", items, onBack }: Props) => {
  ...
  const getItemSeason = (item: any): string => {
    if (item.season !== undefined && item.season !== null && item.season !== '') {
      return String(item.season).replace('期', '');
    }
    ...
    return items
      .filter((item) => {
        ...
        if (filterSeason !== 'all') {
          ...
          if (currentSeason !== filterSeason) return false;
        }
  ...
  }, [items, activeSubCategory, filterSeason, sortOrder]);
    ...
    <div>
      ...
      <div className="mb-6 bg-white p-4 sm:p-5 rounded-xl border border-gray-200 shadow-sm">
        <div className="flex items-center gap-2 mb-4 pb-3 border-b border-gray-100">
          <button 
            onClick={onBack}
            className="p-1.5 rounded-lg border border-gray-200 bg-white text-gray-600 hover:text-pink-600 hover:border-pink-200 shadow-sm transition-colors flex-shrink-0 cursor-pointer"
            title="セクション一覧に戻る"
          >
            <ArrowLeft size={16} />
          </button>
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <div className="flex items-center bg-gray-100 p-0.5 rounded-lg">
              <button 
                onClick={() => setLayout(1)} 
                className={`p-1.5 rounded transition-colors cursor-pointer ${layout === 1 ? "bg-white shadow text-pink-600" : "text-gray-400 hover:text-gray-600"}`}
                title="1列表示"
              >
              ...
              <button 
                onClick={() => setLayout(2)} 
                className={`p-1.5 rounded transition-colors cursor-pointer ${layout === 2 ? "bg-white shadow text-pink-600" : "text-gray-400 hover:text-gray-600"}`}
                title="2列表示"
              >
                <Columns2 size={18} />
              </button>
              <button 
                onClick={() => setLayout(4)} 
                className={`p-1.5 rounded transition-colors cursor-pointer ${layout === 4 ? "bg-white shadow text-pink-600" : "text-gray-400 hover:text-gray-600"}`}
                title="4列表示"
              >
            ...
            <select 
              ...
              className="text-xs sm:text-sm py-1.5 pl-2 pr-8 border border-gray-200 rounded-lg shadow-sm focus:border-pink-300 focus:ring-0 bg-white font-bold text-gray-700 cursor-pointer"
            >
            ...
            {/* 🌟 配信フィルター（サブカテゴリが存在する場合のみプルダウン表示） */}
          ...
          <button
            ...
            className="flex items-center gap-1 px-3 py-1.5 rounded-lg border border-gray-200 bg-white text-xs font-bold text-gray-600 hover:border-pink-200 hover:text-pink-600 transition-colors shadow-sm cursor-pointer"
          >
            <ArrowUpDown size={14} />
