# backend/fetch_introduction_videos.py
import os
import json
...
def fetch_introduction_videos():
    print("🚀 YouTube Data APIを使用して全アップロード動画から『自己紹介』を取得中...")
    ...
    while True:
        ...
        print(f"🔍 スキャン中... {total_scanned} 件走査済み (自己紹介抽出: {len(unique_videos)} 件)")
        ...
        for item in items:
            ...
            # 🌟 タイトルに「自己紹介」が含まれる動画のみを抽出
            if "自己紹介" not in title:
                continue
            ...
            if video_id not in unique_videos:
                ...
                clean_id = f"intro-{upload_date_str}-{video_id}"
    ...
    output_json_path = os.path.join(project_root, "src", "components", "related", "data", "introduction_videos.json")
